...
void relax_edges(const Graph& graph, const Path& curr_path, const std::vector<bool>& consider_params,
                std::unordered_map<std::string, double>& best_sums,   std::priority_queue<std::pair<double, Path>, 
                                    std::vector<std::pair<double, Path>>, std::greater<std::pair<double, Path>>>& pq) {
    ...
    for (const auto& neighbor_name : neighbors) {
        Path new_path = add_edge_to_path(graph, curr_path, curr_vertex_name, neighbor_name, consider_params);
...
Path dijkstra_alg(const Graph& graph, const std::string& start, const std::string& finish,
    ...
    while (!pq.empty()) {
        ...
        relax_edges(graph, curr_path, consider_params, best_sums, pq);
    }
    return Path();
}
...
Path add_edge_to_path(const Graph& graph, const Path& path, const std::string& from_name, const std::string& to_name,
    ...
    for (size_t i = 0; i < edge.params.size(); i++) {
        if (i < new_path.cost.size()) {
            new_path.cost[i] += edge.params[i];
        } else {
            new_path.cost.push_back(edge.params[i]);
        }
    }
    ...
    for (size_t i = 0; i < edge.params.size() && i < edge.weights.size(); i++) {
        if (consider_params[i]) {
            weighted_cost += edge.params[i] * edge.weights[i];
        }
    }
